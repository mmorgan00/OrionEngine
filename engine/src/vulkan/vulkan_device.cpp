...
void vulkan_device_query_swapchain_support(
  ...
  if (format_count != 0) {
    ...
    vkGetPhysicalDeviceSurfaceFormatsKHR(physical_device, surface, &format_count, out_support_info->formats.data());
  }
...
/**
 * @brief Helper function for checking if a given physical device meets requirements
 */
bool physical_device_meets_requirements(VkPhysicalDevice device, VkSurfaceKHR surface, 
  ...
  if (requirements->discrete_gpu) {
    if (properties->deviceType != VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) {
      OE_LOG(LOG_LEVEL_INFO,"Device is not a discrete GPU, and one is required. Skipping.");
      return false;
    }
  }
  ...
  if ((!requirements->graphics ||
    ...
    (requirements->transfer &&
    out_queue_info->transfer_family_index != -1))) {
    OE_LOG(LOG_LEVEL_INFO, "%s Device meets queue requirements.", properties->deviceName);
    OE_LOG(LOG_LEVEL_TRACE, "Graphics Family Index: %i", out_queue_info->graphics_family_index);
    OE_LOG(LOG_LEVEL_TRACE, "Present Family Index:  %i", out_queue_info->present_family_index);
    OE_LOG(LOG_LEVEL_TRACE, "Transfer Family Index: %i", out_queue_info->transfer_family_index);
    OE_LOG(LOG_LEVEL_TRACE, "Compute Family Index:  %i", out_queue_info->compute_family_index);
  }
  ...
  if (requirements->device_extension_names.data()) {
    ...
    if (available_extension_count != 0) {
      ...
      VK_CHECK(vkEnumerateDeviceExtensionProperties(device, 0, &available_extension_count, available_extensions.data()));
      ...
      for (uint32_t i = 0; i < required_extension_count; ++i) {
        ...
        if (!found) {
          OE_LOG(LOG_LEVEL_INFO, "Required extension not found: '%s', skipping device.",
                 requirements->device_extension_names[i]);
          return false;
        }
      }
    }
    OE_LOG(LOG_LEVEL_DEBUG, "All extensions found on %s", properties->deviceName);

    // Sampler anisotropy
    if (requirements->sampler_anisotropy && !features->samplerAnisotropy) {
      OE_LOG(LOG_LEVEL_INFO, "Device does not support samplerAnisotropy, skipping.");
      return false;
    }
...
bool select_physical_device(backend_context* context) {
  ...
  for (uint32_t i = 0; i < physical_device_count; i++) {
    ...
    // We now have the requirements we want, check if between the device properties and features all are supported
...
bool vulkan_device_create_logical_device(backend_context* context) {
  ...
  VK_CHECK(vkCreateDevice(context->device.physical_device, &device_create_info, nullptr, &context->device.logical_device));
