...
VkPresentModeKHR swapchain_select_present_mode(uint32_t present_mode_count, VkPresentModeKHR* available_present_modes) {
...
VkExtent2D swapchain_select_extent(GLFWwindow* window, const VkSurfaceCapabilitiesKHR capabilities) {
...
void vulkan_swapchain_create(backend_context* context) {
  ...
  VkSurfaceFormatKHR surface_format = swapchain_select_surface_format(sc_s.formats);

  VkPresentModeKHR present_mode = swapchain_select_present_mode(sc_s.format_count, sc_s.present_modes);

  VkExtent2D extent = swapchain_select_extent(context->window, sc_s.capabilities);

  uint32_t image_count = sc_s.capabilities.minImageCount + 1; 

  // Make sure we don't have too many images
  if(sc_s.capabilities.maxImageCount > 0 && image_count > sc_s.capabilities.maxImageCount) {
    image_count = sc_s.capabilities.maxImageCount;
  }

  // Everyone's favorite thing to do in vulkan - fill out a struct for instantiation
  ...
  if(context->device.graphics_queue_index != context->device.present_queue_index) {
    ...
    uint32_t queue_family_indices[] = {context->device.graphics_queue_index, context->device.present_queue_index};
  ...
  create_info.oldSwapchain = VK_NULL_HANDLE; // Only a 'recreate swapchain' type of thing
  ...
  vkGetSwapchainImagesKHR(context->device.logical_device, context->swapchain.handle, &image_count, context->swapchain.images.data());
...
void vulkan_swapchain_create_image_views(backend_context* context) {
    ...
    VK_CHECK(vkCreateImageView(context->device.logical_device, &create_info, nullptr, &context->swapchain.views[i]));
  }
